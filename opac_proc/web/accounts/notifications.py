...
def send_confirmation_email(recipient_email):
    ...
    try:
        ...
        sent_results = utils.send_email(
            ...
            render_template('accounts/email/activate.html', confirm_url=confirm_url))
        return sent_results
...
def send_reset_password_email(recipient_email):
    ...
    try:
        ...
        recover_url = url_for('accounts.reset_password_with_token', token=token, _external=True)
        sent_results = utils.send_email(
            recipient_email,
            "Instruções para recuperar sua senha",
            render_template('accounts/email/recover.html', recover_url=recover_url))
