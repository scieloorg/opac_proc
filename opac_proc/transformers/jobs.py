...
def task_journal_update(ids=None):
    ...
    if ids is None:  # update all collections
        ...
        for journal in models.TransformJournal.objects.all():
            issn = getattr(journal, 'scielo_issn', False) or getattr(journal, 'print_issn', False) or getattr(journal, 'eletronic_issn', False)
            if not issn:
        ...
        for oid in ids:
            try:
                ...
                obj.update(must_reprocess=True)
                obj.reload()
                issn = getattr(obj, 'scielo_issn', False) or getattr(obj, 'print_issn', False) or getattr(obj, 'eletronic_issn', False)
                if not issn:
                    raise ValueError(u'Journal sem issn')
                r_queues.enqueue(
                    stage, model,
