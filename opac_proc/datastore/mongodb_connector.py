...
def get_db_connection():
    if config.MONGODB_USER and config.MONGODB_PASS:
        msg = 'Iniciando conexão - com credenciais do banco: mongo://{username}:{password}@{host}:{port}/{db}'.format(
            **config.MONGODB_SETTINGS)
        logger.debug(msg)
    else:
        msg = 'Iniciando conexão - sem credenciais do banco: mongo://{host}:{port}/{db}'.format(
            **config.MONGODB_SETTINGS)
        logger.debug(msg)
    try:
        db = connect(**config.MONGODB_SETTINGS)
    except Exception as e:   # melhorar captura da Exceção
        msg = "Não é possível conectar com banco de dados mongo. %s", str(e)
        logger.error(msg)
    else:
        db_name = get_opac_proc_db_name()
        msg = "Conexão establecida com banco: %s!" % db_name
        logger.debug(msg)
        return db
