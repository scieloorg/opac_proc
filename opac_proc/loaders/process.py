...
class LoadProcess(Process):
    ...
    def reprocess_collection():
        collections = models.LoadCollection(must_reprocess=True)
        for collection in collections:
            self.process_collection(collection.uuid)

    def reprocess_journal():
        journals = models.LoadJournal(must_reprocess=True)
        for journal in journals:
            self.process_journal(self.collection_acronym, journal.code)

    def reprocess_issue():
        issues = models.LoadIssue(must_reprocess=True)
        for issue in issues:
            self.process_issue(self.collection_acronym, issue.code)

    def reprocess_article():
        articles = models.LoadArticle(must_reprocess=True)
    ...
    def process_collection(collection_acronym=None):
        ...
        if self.async:
            ...
            collection = jobs.task_load_collection(collection_acronym)
            collection.reload()
            return collection
    ...
    def process_journal(collection_acronym, issn):
        if self.async:
            ...
            # invocamos a task como funcão normal (sem fila)
            journal = jobs.task_load_journal(
