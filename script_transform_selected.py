...
from opac_proc.transformers.jobs import task_transform_issue, task_transform_article
...
for child in collection['children_ids']:
    print("procurando... issn: ", child['issn'])
    if child['issn'] in target_issn_list:
        print("encontrei o periódico procurado: %s" % child['issn'])
        # dos children_ids, pegamos os articles_ids (pids do artigos)
        articles_ids = child['articles_ids']
        issues_ids = child['issues_ids']
        print("encontrei %s pids de issues do periódico %s para processar" % (len(issues_ids), child['issn']))
        print("encontrei %s pids de artigos do periódico %s para processar" % (len(articles_ids), child['issn']))
        ...
        print("enfilerados %s PIDs de issues do periódico %s" % (enqueued_issues_counts, child['issn']))
        ...
        print("enfilerados %s PIDs de artigos do periódico %s" % (enqueued_articles_counts, child['issn']))
